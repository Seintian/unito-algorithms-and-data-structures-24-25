...
// Define the hash table structure and node structure as opaque types
...
void hash_table_resize(HashTable* table, int new_capacity);                        // Resize the hash table to a new capacity
float hash_table_load_factor(const HashTable* table);                              // Calculate the current load factor
void hash_table_replace(HashTable* table, const void* key, const void* new_value); // Replace the value for an existing key
void hash_table_clear(HashTable* table);                                           // Clear all elements from the hash table
void hash_table_map(HashTable* table, void (*func)(void* key, void* value));       // Apply a function to each key-value pair
