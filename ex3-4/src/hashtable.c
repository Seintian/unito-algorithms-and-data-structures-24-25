...
static HashNode* create_node(const void* key, const void* value) {
...
void hash_table_resize(HashTable* table, int new_capacity) {
    ...
    for (int i = 0; i < table->capacity; i++) {
        HashNode* node = table->buckets[i];
        while (node) {
            unsigned long new_index = table->hash_func(node->key) % new_capacity;
...
int hash_table_equals(const HashTable* table1, const HashTable* table2) {
    if (table1->size != table2->size || table1->capacity != table2->capacity) return 0;
    for (int i = 0; i < table1->capacity; i++) {
        HashNode* node = table1->buckets[i];
        while (node) {
            const void* value = hash_table_get(table2, node->key);
            if (!value || table1->compare(value, node->value) != 0) return 0;
            node = node->next;
        }
    }
    return 1;
